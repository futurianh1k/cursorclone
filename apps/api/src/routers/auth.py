...
async def login(
    ...
    allowed, rate_limit_msg = await rate_limit_service.check_login_rate_limit(
        login_login_request.email.lower(),
        ip_address,
    )
    if not allowed:
    ...
    # 사용자 조회
    user = await db.execute(
        select(UserModel).where(UserModel.email == login_login_request.email.lower())
    )
    user = user.scalar_one_or_none()
    
    if not user:
        # 실패 기록
        await rate_limit_service.record_login_attempt(login_login_request.email.lower(), ip_address, success=False)
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    # 로그인 성공
    await rate_limit_service.record_login_attempt(login_login_request.email.lower(), ip_address, success=True)
